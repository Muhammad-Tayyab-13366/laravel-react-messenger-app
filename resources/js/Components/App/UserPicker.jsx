...
export default function UserPickeer({value, options, onSelect}) {
    ...
    return (
        ...
        <Combobox value={selected} onChange={onSelected} multiple >
            <div className="relative mt-1">
                <div className="relative w-full cursor-default overflow-hidden rounded-lg text-left shadow-md focus-outline-none focus-visible:ring-2 focus:visible:ring-white/75 focus-visible:ring-offset-2 focus-visible:ring-offest-teal-300 sm:text-sm">
                    <Combobox.Input className="border-gray-300 darl:border-gray-700 dark:bg:gray
                    ...
                    displayValue={(persons) => {
                        ...
                        return persons.length > 0 ? `${persons.length} users selected` : ""
                    }}
                    ...
                    <Combobox.Button className="absolute insect-y-0 right-0 top-3 text-black-400 flex items-center pr-2">
        ...
        {selected && (
            <div className="flex flex-wrap gap-2 mt-3">
                {selected.map((person) => {
                    return (
                        <div className="badge badge-primary gap-2 w-auto">{person.name}</div>
                    )
