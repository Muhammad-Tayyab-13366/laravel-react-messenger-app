...
const MessageOptionsDropdown = ({message}) => {
    ...
    const onMessageDelete = () => {
        axios.post(route("message.destroy", message.id))
        .then((res) => {
            ...
            <Menu as="div" className="relative inline-block text-left">
                <div>
                    <Menu.Button className="flex justify-center items-center w-8 h-8 rounded-full hover:bg-black/40" >
                        <EllipsisVerticalIcon className="h-5 w-5 z-50" />
                    </Menu.Button>
                </div>
                <Transition
                    as={Fragment}
                ...
                >
                    <Menu.Items className="absolute right-0 mt-2 w-48  rounded-md bg-black/30 shadow-lg ">
                        <div className="px-1 py-1 ">
                            <Menu.Item>
                                ...
                                <button
                                    ...
                                    className={`${
                                    active ? 'bg-black/30 text-white' : 'text-white'
